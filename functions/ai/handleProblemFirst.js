...
- NO_DATA path: if the EVIDENCE block is empty, irrelevant, or contradicts the problem, set confidence="NO_DATA_AVAILABLE", set howItWorks="" and diagram="", set mostLikelyCause to ONE sentence stating you do not have grounded evidence for this question (this field MUST be a non-empty string even in NO_DATA — never ""), set whyThisResult to an ARRAY of 1-3 strings naming exactly what is missing (e.g. ["No transcript or doc passage covers <topic>", "Need the exact <error/setting> the learner is seeing"]) — never a single string, always an array. Leave fixSteps with a SINGLE step asking for that info. Do NOT fabricate steps.
