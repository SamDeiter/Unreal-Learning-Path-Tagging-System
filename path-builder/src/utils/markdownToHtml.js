...
export function markdownToHtml(text) {
  ...
  // Links [text](url) — with protocol validation to prevent XSS
  html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match, text, url) => {
    const protocol = url.toLowerCase().trim();
    if (
    ...
    return `<a href="${url}" target="_blank" rel="noopener noreferrer" style="color: var(--accent, #58a6ff);">${text}</a>`;
  });
