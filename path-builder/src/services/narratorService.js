  ...
  blueprint: [
    {
      ...
      expectedResult:
        "When you walk into the collision box in Play mode, the Point Light should turn on/off. The Event Graph should show a connected chain from Overlap → Set Visibility with no compile errors.",
    },
    {
      ...
      expectedResult:
        "After pressing Play, wait 3 seconds — the variables should snap back to their defaults. Print String nodes on each Set will confirm the reset fires. The Blueprint should compile without errors.",
    },
  ],
  materials: [
    {
      ...
      expectedResult:
        "The cube should display your texture on its surface with an orange glow from the emissive channel. In 'Lit' mode, the emissive color should be visible even in shadow. The Material should show no errors in the Stats panel.",
    },
    {
      ...
      expectedResult:
        "Changing the color slider on the Material Instance should immediately update the mesh's appearance in the viewport — no recompile needed. Both parameters should appear as editable fields in the Instance's Details panel.",
    },
  ],
  lighting: [
    {
      ...
      expectedResult:
        "The Rect Light should cast soft, directional shadows with a warm rectangular pool of light on the floor. Compared to the Point Light, the Rect Light shadows should be noticeably softer with more realistic falloff at the edges.",
    },
    {
      ...
      expectedResult:
        "Your subject should have a bright side (key), a dimly lit opposite side (fill) with soft shadows, and a bright edge outline on the back (rim). The overall look should feel cinematic with clear depth separation from the background.",
    },
  ],
  lumen: [
    {
      ...
      expectedResult:
        "Nearby white walls should pick up a tinted color from the Rect Light's bounce. Moving the light should update the indirect illumination within 1-2 seconds. Hardware RT mode should show sharper, more accurate bounces with less noise.",
    },
    {
      ...
      expectedResult:
        "Light should visibly bleed around the doorframe edges into the dark hallway. The emissive mesh should cast soft colored light onto nearby surfaces — you'll see the floor and walls near it glow with the emissive color.",
    },
  ],
  animation: [
    {
      ...
      expectedResult:
        "In the Blend Space preview, dragging the slider from 0 to 300 should smoothly transition from Idle to Walk. In Play mode, the character should blend between animations based on movement speed — standing still plays Idle, walking plays Walk.",
    },
    {
      ...
      expectedResult:
        "When the montage plays and reaches the halfway mark, 'FootstepSound' should appear in the Output Log via Print String. The Notify marker should be visible as a triangle on the Montage timeline at the exact position you placed it.",
    },
  ],
  niagara: [
    {
      ...
      expectedResult:
        "Particles should now live longer (1-3 seconds instead of default), creating taller fountain arcs. The sprites should be visibly smaller (5-15 vs default). In the level viewport, you should see a continuous stream of small particles rising and falling.",
    },
    {
      ...
      expectedResult:
        "Particles should visibly swirl and curl in organic patterns instead of falling straight down. At Strength 200 the effect should be dramatic — particles will loop and spiral. Reducing Frequency makes larger, lazier swirls.",
    },
  ],
  landscape: [
    {
      ...
      expectedResult:
        "You should see a flat green landscape with a visibly sunken channel carved through it. The channel should have smooth, eroded edges (not sharp cuts). The water material should appear only on the painted channel area, distinct from the surrounding terrain.",
    },
    {
      ...
      expectedResult:
        "Steep slopes should auto-paint as Rock, flat areas as Grass, and your manually painted Sand should appear where you chose. Each layer should blend smoothly at boundaries — no hard seams between Grass and Rock transitions.",
    },
  ],
  mesh: [
    {
      ...
      expectedResult:
        "In the mesh editor, the LOD picker should show LOD0 and LOD1 with different triangle counts. In the viewport, zooming out past the 50% screen threshold should visibly switch to a simpler mesh version. Use 'r.StaticMeshLODDistanceScale 1' to verify.",
    },
  ],
  texture: [
    {
      ...
      expectedResult:
        "The texture should render clearly on the mesh with BC7 compression (minimal artifacts). With LOD Bias 1, the texture should appear slightly blurrier at distance (using a lower mip). The Material's Stats should show the texture memory size.",
    },
  ],
  character: [
    {
      ...
      expectedResult:
        "In Play mode, your character should move noticeably faster than default (600 → 400). Jumping should feel floatier with higher arc (Z Velocity 600). While airborne, WASD should steer the character mid-jump thanks to Air Control 0.3.",
    },
  ],
  sequencer: [
    {
      ...
      expectedResult:
        "When you press Play in the Sequencer timeline, the light should gradually brighten from completely off to full intensity over exactly 4 seconds. The keyframe curve in the Sequencer should show a smooth ramp from 0 to 20. The scene should start dark and end lit.",
    },
  ],
  umg: [
    {
      ...
      expectedResult:
        "In the Widget Designer preview, you should see 'Health: 100' text above a green progress bar. Changing the HealthPercent default value (0.0-1.0) should immediately update the bar fill. The binding icon next to Percent should show as linked (chain icon).",
    },
  ],
  performance: [
    {
      ...
      expectedResult:
        "You should see an FPS counter in the top-left and a 4-bar breakdown (Game/Draw/GPU/RHIT). If any bar exceeds 16.6ms, that thread is your bottleneck. The scenerendering stats should highlight which pass (Base Pass, Shadows, Translucency) costs the most ms.",
    },
  ],
  nanite: [
    {
      ...
      expectedResult:
        "With Nanite ON, 50 instances of a 1M-tri mesh should maintain near the same FPS as a single instance. With Nanite OFF, FPS should drop significantly. The Nanite visualization should show colored triangle clusters that change density based on distance.",
    },
  ],
  rendering: [
    {
      ...
      expectedResult:
        "Shadows should remain sharp and detailed even at far distances (unlike Cascaded Shadow Maps which get blurry). Close-up shadows should have clean edges without visible cascade boundaries. The stat overlay should show VSM memory and page usage.",
    },
  ],
  component: [
    {
      ...
      expectedResult:
        "In the Blueprint viewport, all 3 components should be visible on the actor. Moving the root Static Mesh should move the particle and audio with it (they follow the parent). The hierarchy panel should show Audio and Particles indented under the mesh.",
    },
  ],
  actor: [
    {
      ...
      expectedResult:
        "A new Blueprint asset should appear in your Content Browser. Dropping multiple instances into the level should each show the same mesh+light setup. The Spot Light cone should visibly illuminate the mesh in Lit viewport mode.",
    },
  ],
  foliage: [
    {
      ...
      expectedResult:
        "Trees/bushes should scatter naturally across the painted area with slight size variation (0.8x-1.2x). On slopes, they should tilt to match the terrain angle. The Foliage panel should show the total instance count increasing as you paint.",
    },
  ],
  plugin: [
    {
      ...
      expectedResult:
        "After restart, the Place Actors panel should list 'Water Body Lake'. Placing it should auto-generate a translucent water surface that conforms to your landscape. You should see reflections and shoreline foam where the water meets terrain.",
    },
  ],
};
