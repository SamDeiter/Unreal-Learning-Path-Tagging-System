...
describe("TagGraphService", () => {
  describe("constructor and data loading", () => {
    ...
    it("should build a term index for text matching", () => {
      ...
      expect(tagGraphService.phraseIndex.length + tagGraphService.termMap.size).toBeGreaterThan(0);
    });
  ...
  describe("_buildTermIndex", () => {
    ...
    it("should include tag_id suffix terms", () => {
      const suffixEntriesInPhrases = tagGraphService.phraseIndex.filter((e) => e.termType === "tag_id_suffix");
      const suffixEntriesInMap = Array.from(tagGraphService.termMap.values())
        .flat()
        .filter((e) => e.termType === "tag_id_suffix");
      expect(suffixEntriesInPhrases.length + suffixEntriesInMap.length).toBeGreaterThan(0);
    });

    it("should include display_name terms", () => {
      const dnEntriesInPhrases = tagGraphService.phraseIndex.filter((e) => e.termType === "display_name");
      const dnEntriesInMap = Array.from(tagGraphService.termMap.values())
        .flat()
        .filter((e) => e.termType === "display_name");
      expect(dnEntriesInPhrases.length + dnEntriesInMap.length).toBeGreaterThan(0);
    });
  });
});
