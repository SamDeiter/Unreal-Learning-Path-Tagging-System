 ...
 *
 * Security: Isolated per-user to prevent IDOR where one user could
 * enumerate another's API usage history.
 */
async function syncDayToFirestore(dateKey, dayData) {
  try {
    ...
    // Isolated path: users/{uid}/token_usage/{date}
...
export async function fetchCloudStats(days = 30) {
  try {
    ...
    // Isolated path: users/{uid}/token_usage
    const q = query(
