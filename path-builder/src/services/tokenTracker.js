...
export async function syncDayToFirestore(dateKey, dayData) {
  try {
    ...
    if (!uid) {
      console.warn("[TokenTracker] No authenticated user found for Firestore sync.");
      return;
    }
    const db = getFirestore(app);
    const docRef = doc(db, "users", uid, "token_usage", dateKey);
    await setDoc(
      docRef,
      {
        date: dateKey,
...
export async function fetchCloudStats(days = 30) {
  try {
    ...
    const uid = auth.currentUser?.uid;
    if (!uid) {
      console.warn("[TokenTracker] No authenticated user found to fetch cloud stats.");
      return [];
    }
    const db = getFirestore(app);
    const q = query(
      collection(db, "users", uid, "token_usage"),
      orderBy("date", "desc"),
      limit(days)
    );
