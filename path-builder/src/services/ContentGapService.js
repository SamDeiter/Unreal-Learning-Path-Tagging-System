...
// Cache for normalized course data to avoid redundant lowercasing and array merging
const courseCache = new WeakMap();

/**
 * Extracts and caches normalized metadata for a course.
 */
function getNormalizedCourseData(course) {
  if (courseCache.has(course)) return courseCache.get(course);
  ...
  // Join tags into a single blob for faster multi-keyword searching
  const tagsBlob = allTags.join(" | ");

  const data = { title, allTags, tagsBlob };
...
/**
 * Gets normalized scoring rules for a persona.
 */
function getNormalizedRules(personaId) {
  if (personaRulesCache.has(personaId)) return personaRulesCache.get(personaId);
...
export function analyzeGaps(personaId, courses = [], _tags = []) {
  ...
  const scored = courses.map((course) => {
    const { title, tagsBlob } = getNormalizedCourseData(course);
...
export function getRelevanceBadge(course, personaId) {
  ...
  for (const kw of rules.boostKeywords) {
    ...
    else if (tagsBlob.includes(kw)) score += 3;
  }

  for (const kw of rules.penaltyKeywords) {
    if (title.includes(kw) || tagsBlob.includes(kw)) {
      score -= 10;
      hasPenalty = true;
    }
  }
