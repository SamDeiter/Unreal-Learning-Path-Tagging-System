...
export async function runSearchPipeline(query, options = {}) {
  ...
  try {
    ...
    if (
      ...
      // Pre-tokenize to avoid O(N^2) redundant tokenization
      const tokenizedPassages = retrievedPassages.map((p) => ({
        ...p,
        _tokens: tokenize(p.text || ""),
      }));

      const semanticDeduped = [];
      for (const p of tokenizedPassages) {
        const isDupe = semanticDeduped.some(
          (kept) => wordJaccard(kept._tokens, p._tokens) > 0.7
        );
        if (!isDupe) semanticDeduped.push(p);
      }
      retrievedPassages = semanticDeduped.map(({ _tokens: _, ...p }) => p);
