...
import { getWordSet, jaccardFromSets } from "../utils/textSimilarity";
...
export async function runSearchPipeline(query, options = {}) {
  ...
  try {
    ...
    if (
      ...
      // Pre-calculate word sets to avoid redundant tokenization in the O(N^2) loop
      ...
      // Remove wordSet before returning to keep response objects clean
      retrievedPassages = semanticDeduped.map(({ wordSet: _UNUSED_SET, ...p }) => p);
