...
import { wordJaccardFromSets, getWordSet } from "../utils/textSimilarity";
...
export async function runSearchPipeline(query, options = {}) {
  ...
  try {
    ...
    if (
      ...
      // Pre-calculate word sets to avoid redundant tokenization in the nested loop
      for (const p of retrievedPassages) {
