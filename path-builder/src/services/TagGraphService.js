...
class TagGraphService {
  ...
  /**
   * Internal helper to get cached or build course metadata
   */
  _getCourseMetadata(course) {
    ...
    metadata = { tagSet, suffixMap, geminiTags: (course.gemini_system_tags || []).map(t => t.toLowerCase()) };
    this._courseMetadataCache.set(course, metadata);
    return metadata;
  }

  /**
   * Internal helper to get cached or build BFS graph expansion for a tag
   */
  _getTagExpansion(tagId) {
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          const creditMultiplier = dirWeight * Math.pow(HOP_ATTENUATION, hops) * (edge.weight || 0.5);
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Deduplicate and normalize target IDs for this batch
    ...
    for (const tagId of uniqueTargets) {
      ...
      const expansion = this._getTagExpansion(tagId);

      for (const node of expansion) {
        let matched = false;
        let matchedTagId = node.id;

        if (tagSet.has(node.id)) {
          matched = true;
        } else if (node.suffix && node.suffix.length > 2) {
          const m = suffixMap.get(node.suffix);
          if (m) {
            matched = true;
            matchedTagId = m;
          }
        ...
        if (matched) {
          const credit = 5 * node.creditMultiplier;
