...
class TagGraphService {
  constructor() {
    ...
    // Identity-based cache for course metadata (normalized tags and suffixes)
    this._courseCache = new WeakMap();

    // Cache for BFS graph expansions from target tags
  ...
  /**
   * Internal helper to normalize course tags once.
   */
  _getNormalizedCourseData(course) {
    if (this._courseCache.has(course)) return this._courseCache.get(course);
    ...
    allCourseTags.forEach((t) => {
      const parts = t.split(".");
      if (parts.length > 1) suffixSet.add(parts.pop());
    });

    const data = { allCourseTags, tagSet, suffixSet };
    this._courseCache.set(course, data);
    return data;
  }

  /**
   * Internal helper to perform BFS expansion for a tag and cache it.
   */
  _getGraphExpansion(tagId) {
    ...
    const reachable = []; // Array of { tagId, suffix, credit, path }
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          reachable.push({
            tagId: edge.neighborId.toLowerCase(),
            suffix: edge.neighborId.toLowerCase().split(".").pop(),
            credit,
            path,
          });
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Use cached course metadata (O(1) after first call)
    const { allCourseTags, tagSet, suffixSet } = this._getNormalizedCourseData(course);
    ...
    for (const target of targetSet) {
      // Exact match on full tag ID (O(1))
      ...
      // Check suffix match (O(1) via suffixSet)
      const targetSuffix = target.split(".").pop();
      if (targetSuffix.length > 2 && suffixSet.has(targetSuffix)) {
        directOverlap += 15;
        // Find the actual full tag in allCourseTags for the contributor report
        const fullTag = allCourseTags.find(ct => ct.endsWith(`.${targetSuffix}`));
        topContributors.push({
          sourceQueryTagId: target,
          targetCourseTagId: fullTag || targetSuffix,
          path: [],
          contribution: 15,
        });
      }
        ...
        if (
          geminiTags.some(
            (gt) => {
              ...
              return gtl === targetSuffix || gtl.includes(targetSuffix);
            }
          )
        ) {
          geminiBonus += 10;
    ...
    for (const tagId of targetTagIds) {
      ...
      // Use cached BFS expansion results (O(1) lookup + O(G) iteration over graph neighbors)
      const reachable = this._getGraphExpansion(tagId);

      for (const node of reachable) {
        let matched = false;
        if (tagSet.has(node.tagId)) {
          matched = true;
        } else if (node.suffix.length > 2 && suffixSet.has(node.suffix)) {
          matched = true;
        }

        if (matched) {
          tagGraphCredit += node.credit;
          topContributors.push({
            sourceQueryTagId: tagId,
            targetCourseTagId: node.tagId,
            path: node.path,
            contribution: Math.round(node.credit * 100) / 100,
          });
        }
      }
