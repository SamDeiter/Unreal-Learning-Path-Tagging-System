...
class TagGraphService {
  constructor() {
    ...
    // Cache for BFS expansions (id, weight, path, suffix) per query tag
    this._expansionCache = new Map();
    // Identity-based cache for normalized course metadata
  ...
  /**
   * Internal helper to expand a tag in the graph up to 2 hops.
   * Caches results in this._expansionCache.
   */
  _getGraphExpansion(tagId) {
    ...
    const expansion = []; // Array of { id, credit, path, suffix }
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          expansion.push({
            id: edge.neighborId,
            neighborTag: edge.neighborId.toLowerCase(),
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Use Identity-based cache for normalized course metadata
    ...
    if (!metadata) {
      ...
      metadata = {
        ...
        suffixSet: new Set(allCourseTags.map((t) => t.split(".").pop())),
        geminiTags: (course.gemini_system_tags || []).map((t) => t.toLowerCase()),
      };
      this._courseMetadataCache.set(course, metadata);
    }

    const targetLowerIds = targetTagIds.map((t) => t.toLowerCase());
    const topContributors = [];

    // ---- 1. Direct tag matches (highest weight: 25 pts each) ----
    let directOverlap = 0;
    for (const target of targetLowerIds) {
      if (metadata.tagSet.has(target)) {
        directOverlap += 25;
      ...
      if (suffix && suffix.length > 2 && metadata.suffixSet.has(suffix)) {
        directOverlap += 15;
        // Find WHICH course tag matched the suffix for the contributor list
        const matchedTag = metadata.allCourseTags.find((ct) => ct.endsWith("." + suffix) || ct === suffix);
        topContributors.push({
          sourceQueryTagId: target,
    ...
    for (const tagId of targetTagIds) {
      ...
      for (const entry of expansion) {
        let matched = false;
        if (metadata.tagSet.has(entry.neighborTag)) {
          matched = true;
        } else if (entry.neighborSuffix.length > 2 && metadata.suffixSet.has(entry.neighborSuffix)) {
          matched = true;
        }
