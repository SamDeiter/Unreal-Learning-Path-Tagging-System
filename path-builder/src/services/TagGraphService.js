...
class TagGraphService {
  constructor() {
    ...
    this._graphExpansionCache = new Map();
  ...
  /**
   * Internal helper: Get or build cached metadata for a course.
   * Caches normalized tags and sets to avoid redundant processing.
   */
  _getCourseMetadata(course) {
    ...
    for (const tag of tagSet) {
      ...
      if (s && s.length > 2) suffixMap.set(s, tag);
    }

    const metadata = { allCourseTags, tagSet, suffixMap, geminiTags };
    this._courseMetadataCache.set(course, metadata);
    return metadata;
  }

  /**
   * Internal helper: Get or build cached graph expansion for a tag.
   * Moves expensive BFS out of the per-course loop.
   */
  _getGraphExpansion(tagId) {
    if (this._graphExpansionCache.has(tagId)) {
      return this._graphExpansionCache.get(tagId);
    }

    const MAX_HOPS = 2;
    const HOP_ATTENUATION = 0.5;
    const expansion = []; // Array of { neighborId, suffix, credit, path }
    ...
    this._graphExpansionCache.set(tagId, expansion);
    return expansion;
  }
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // 1. Get cached course metadata
    const { tagSet, suffixMap, geminiTags: normalizedGeminiTags } = this._getCourseMetadata(course);
    ...
    for (const tagId of targetTagIds) {
      ...
      for (const entry of expansion) {
        ...
        if (tagSet.has(entry.neighborId)) {
          matchedTagId = entry.neighborId;
        } else if (entry.suffix.length > 2 && suffixMap.has(entry.suffix)) {
          matchedTagId = suffixMap.get(entry.suffix);
        }
  ...
  /**
   * Clear the caches. Call between query batches.
   */
  clearRelatedCache() {
    this._relatedCache.clear();
    this._graphExpansionCache.clear();
    // No need to clear WeakMap courseMetadataCache as it's self-cleaning
  }
