...
class TagGraphService {
  constructor() {
    ...
    this._courseDataCache = new WeakMap();
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // ---- 0. Identity-based course normalization cache ----
    let courseData = this._courseDataCache.get(course);
    if (!courseData) {
      ...
      allCourseTags.forEach(t => {
        const s = t.split(".").pop();
        if (s && !suffixMap.has(s)) suffixMap.set(s, t);
      });
      const geminiTags = (course.gemini_system_tags || []).map((t) => t.toLowerCase());

      courseData = { allCourseTags, tagSet, suffixMap, geminiTags };
      this._courseDataCache.set(course, courseData);
    }

    const { tagSet, suffixMap, geminiTags } = courseData;

    // Use a simple cache for the target tags to avoid re-normalizing for every course in a batch
    if (this._lastTargetTagIds !== targetTagIds) {
      this._lastTargetTagIds = targetTagIds;
      this._targetTagIdsNormalized = targetTagIds.map((t) => t.toLowerCase());
      this._targetSet = new Set(this._targetTagIdsNormalized);
    }
    const targetTagIdsNormalized = this._targetTagIdsNormalized;
    ...
    for (const target of targetSet) {
      ...
      // Check suffix match: "lumen" matches course tag "rendering.lumen" (O(1) lookup)
      const suffix = target.split(".").pop();
      const actualTag = suffixMap.get(suffix);
      if (suffix && suffix.length > 2 && actualTag) {
        directOverlap += 15;
        topContributors.push({
          sourceQueryTagId: target,
          targetCourseTagId: actualTag,
          path: [],
          contribution: 15,
        });
      }
    ...
    for (const target of targetSet) {
      ...
      // O(N) but small array (geminiTags usually < 5)
      if (
        geminiTags.some(
    ...
    for (const tagId of targetTagIdsNormalized) {
      // --- BFS Expansion Cache: Avoid re-running BFS for the same query tag ---
      let expansion = this._expansionCache.get(tagId);
      if (!expansion) {
        expansion = {
          matches: [], // Array of { neighborId, neighborSuffix, credit, path }
        };
        ...
        while (frontier.length > 0) {
          ...
          for (const { id, hops } of frontier) {
            ...
            for (const edge of [...outgoing, ...incoming]) {
              ...
              expansion.matches.push({
                neighborId: edge.neighborId.toLowerCase(),
      ...
      // --- Apply cached expansion to this course ---
      let tagGraphCredit = 0;
      for (const m of expansion.matches) {
        let matched = false;
        if (tagSet.has(m.neighborId)) {
          matched = true;
        } else if (m.neighborSuffix.length > 2 && suffixMap.has(m.neighborSuffix)) {
          matched = true;
        }

        if (matched) {
          tagGraphCredit += m.credit;
          topContributors.push({
            sourceQueryTagId: tagId,
            targetCourseTagId: m.neighborId,
            path: m.path,
            contribution: Math.round(m.credit * 100) / 100,
          });
        }
      }
