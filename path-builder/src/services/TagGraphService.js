...
class TagGraphService {
  constructor() {
    ...
    // V2 Optimization: Cache for BFS expansions and course metadata
  ...
  /**
   * Internal helper to expand a tag 2-hops and cache results
   */
  _getBfsExpansion(tagId) {
    const normId = tagId.toLowerCase();
    if (this._bfsCache.has(normId)) return this._bfsCache.get(normId);

    const expansion = [];
    const visited = new Set([normId]);
    const parentMap = new Map();
    let frontier = [{ id: normId, hops: 0 }];
    ...
    this._bfsCache.set(normId, expansion);
    return expansion;
  }
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // 1. Get or build course metadata (memoized by object identity)
    ...
    if (!meta) {
      ...
      for (const t of tagSet) {
        const s = t.split(".").pop();
        if (s && s.length > 2) {
          if (!suffixMap.has(s)) suffixMap.set(s, t);
        }
      }
    ...
    const { tagSet, suffixMap, geminiTags } = meta;
    const uniqueTargetTags = [...new Set(targetTagIds.map(t => t.toLowerCase()))];
    const topContributors = [];

    // ---- 2. Direct tag matches (highest weight: 25 pts each) ----
    let directOverlap = 0;
    for (const target of uniqueTargetTags) {
      ...
      if (suffix.length > 2 && suffixMap.has(suffix)) {
        const matchedCourseTag = suffixMap.get(suffix);
        directOverlap += 15;
        topContributors.push({
          sourceQueryTagId: target,
    ...
    // ---- 3. Gemini bonus (AI-curated, high quality) ----
    let geminiBonus = 0;
    for (const target of uniqueTargetTags) {
    ...
    // ---- 4. Graph propagation (secondary, capped) ----
    ...
    for (const tagId of uniqueTargetTags) {
      ...
      for (const exp of expansion) {
        ...
        if (tagSet.has(exp.id)) {
          matchedTagId = exp.id;
        } else if (exp.suffix.length > 2 && suffixMap.has(exp.suffix)) {
          matchedTagId = suffixMap.get(exp.suffix);
        }

        if (matchedTagId) {
          const credit = 5 * exp.weight;
