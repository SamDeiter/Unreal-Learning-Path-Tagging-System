...
class TagGraphService {
  constructor() {
    ...
    // - _courseMetadata: Identity-based cache for normalized course tag sets and suffixes
    // - _bfsCache: Cache for graph expansion results (reachable tags + contribution paths)
  ...
  /**
   * Performance: Pre-calculate and cache course metadata (normalized tag sets and suffixes).
   * @private
   */
  _getCourseMetadata(course) {
    ...
    for (const t of tagSet) {
      ...
      if (suffix && suffix.length > 2) suffixSet.add(suffix);
    }

    const metadata = { tagSet, suffixSet, geminiTags: (course.gemini_system_tags || []).map(t => t.toLowerCase()) };
    this._courseMetadata.set(course, metadata);
    return metadata;
  }

  /**
   * Performance: Pre-calculate and cache graph expansion for a given tagId.
   * @private
   */
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Bolt Optimization: Deduplicate target tag IDs and normalize
    const targetTagIdList = [...new Set(targetTagIds)].map(t => t.toLowerCase());

    // Bolt Optimization: Identity-based metadata caching
    const { tagSet, suffixSet, geminiTags } = this._getCourseMetadata(course);
    const topContributors = [];

    // ---- 1. Direct tag matches (highest weight: 25 pts each) ----
    let directOverlap = 0;
    for (const target of targetTagIdList) {
      ...
      if (targetSuffix.length > 2 && suffixSet.has(targetSuffix)) {
        directOverlap += 15;
        // Optimization: Find the specific course tag for the contributor entry
        const matchedTag = [...tagSet].find(ct => ct.endsWith(`.${targetSuffix}`) || ct === targetSuffix);
        topContributors.push({
          sourceQueryTagId: target,
          targetCourseTagId: matchedTag || `*${targetSuffix}`,
          path: [],
          contribution: 15,
        });
      }
    ...
    for (const tagId of targetTagIdList) {
      let tagGraphCredit = 0;

      // Bolt Optimization: BFS Expansion caching
      const reachable = this._getBfsExpansion(tagId);

      for (const entry of reachable) {
        let matched = false;
        if (tagSet.has(entry.neighborId)) {
          matched = true;
        } else if (entry.neighborSuffix.length > 2 && suffixSet.has(entry.neighborSuffix)) {
          matched = true;
        }
        ...
        if (matched) {
          ...
          topContributors.push({
            ...
            path: [...entry.path], // Clone path to prevent accidental mutations
