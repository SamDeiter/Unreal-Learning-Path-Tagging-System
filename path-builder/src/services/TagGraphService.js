...
class TagGraphService {
  constructor() {
    ...
    this._courseCache = new WeakMap(); // Identity-based cache for course metadata
    this._expansionCache = new Map(); // Cache for BFS expansion results
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // 0. Use identity-based cache for course tag normalization
    let courseMetadata = this._courseCache.get(course);
    if (!courseMetadata) {
      const allCourseTags = [
      ...
      this._courseCache.set(course, courseMetadata);
    }

    const { allTags: allCourseTags, tagSet: courseTagSet, suffixSet: courseSuffixSet, geminiTags } = courseMetadata;
    ...
    for (const target of targetSet) {
      ...
      if (suffix.length > 2 && courseSuffixSet.has(suffix)) {
        directOverlap += 15;
        // Find the actual full tag for the contributor list
        const matchedFullTag = allCourseTags.find(ct => ct.endsWith("." + suffix)) || target;
        topContributors.push({
          sourceQueryTagId: target,
          targetCourseTagId: matchedFullTag,
          path: [],
          contribution: 15,
        });
      }
    ...
    for (const tagId of targetTagIds) {
      // 3a. Cache BFS expansion results for the query tag
      ...
      // 3b. Score course against pre-calculated expansion
      let tagGraphCredit = 0;
      for (const item of expansion) {
        let matched = false;
        if (courseTagSet.has(item.id)) {
          matched = true;
        } else if (item.suffix.length > 2 && courseSuffixSet.has(item.suffix)) {
          matched = true;
        }

        if (matched) {
          tagGraphCredit += item.credit;
          topContributors.push({
            sourceQueryTagId: tagId,
            targetCourseTagId: item.id,
            path: item.path,
            contribution: Math.round(item.credit * 100) / 100,
          });
        }
      }
