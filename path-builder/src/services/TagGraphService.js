...
class TagGraphService {
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Cache clearing check: if targetTagIds changed (by reference), clear query-level caches
    ...
    for (const target of uniqueTargets) {
      // Exact match on full tag ID (O(1) lookup)
      ...
      // Suffix match: "lumen" matches course tag "rendering.lumen"
      const suffix = target.split(".").pop();
      if (suffix && suffix.length > 2) {
        const matchedTag = courseSuffixMap.get(suffix);
        if (matchedTag) {
          directOverlap += 15;
          topContributors.push({
            sourceQueryTagId: target,
    ...
    for (const tagId of uniqueTargets) {
      ...
      const expansions = this._getExpansion(tagId);

      for (const exp of expansions) {
        let matched = false;
        let matchedId = exp.id;

        // Check if expanded tag (or its suffix) exists in the course
        if (courseTagSet.has(exp.id)) {
          matched = true;
        } else if (exp.suffix.length > 2) {
          const suffixMatch = courseSuffixMap.get(exp.suffix);
          if (suffixMatch) {
            matched = true;
            matchedId = suffixMatch;
          }
        }
        ...
        if (matched) {
          ...
          topContributors.push({
            ...
            path: exp.path,
            contribution: Math.round(exp.credit * 100) / 100,
          });
        }
      }
  ...
  /**
   * Internal: Get or build cached graph expansion for a query tag
   */
  _getExpansion(tagId) {
    const cached = this._expansionCache.get(tagId);
    if (cached) return cached;

    const expansions = []; // List of { id, credit, path, suffix }
    const MAX_HOPS = 2;
  ...
  /**
   * Internal: Get or build cached metadata for a course
   */
  ...
  /**
   * Clear the related-tag and expansion caches. Call between query batches.
   */
  clearRelatedCache() {
