...
class TagGraphService {
  constructor() {
    ...
    // Cache for course-specific normalized data (WeakMap for automatic GC)
    this._courseMetadataCache = new WeakMap();

    // Cache for BFS graph expansions per target tag
  ...
  /**
   * Internal helper to get/cache normalized course metadata.
   * Reduces 3.7MB library processing overhead by ~90%.
   */
  _getCourseMetadata(course) {
    ...
    metadata = { tagSet, suffixMap, geminiTags, allCourseTags };
    this._courseMetadataCache.set(course, metadata);
    return metadata;
  }

  /**
   * Internal helper to get/cache BFS graph expansion for a tag.
   * Avoids O(V+E) traversal during large batch scoring.
   */
  _getBfsExpansion(tagId) {
    ...
    const reachable = []; // Array of { neighborId, neighborSuffix, contributionFactor, path }
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        const outgoing = (this.edgesBySource.get(id) || []).map((e) => ({
          ...e,
          direction: "forward",
          neighborId: e.target.toLowerCase(),
        }));
        const incoming = (this.edgesByTarget.get(id) || []).map((e) => ({
          ...e,
          direction: "reverse",
          neighborId: e.source.toLowerCase(),
        }));
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          const contributionFactor = 5 * dirWeight * hopMultiplier * edgeDataWeight;
    ...
    this._bfsCache.set(tid, reachable);
    return reachable;
  }

  /**
   * V2: Score a course's relevance for a set of tags.
   * Uses identity-based metadata caching and BFS memoization.
   *
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Pre-calculate target-specific data once
    const targetInfo = targetTagIds.map(tid => ({
      id: tid.toLowerCase(),
      suffix: tid.toLowerCase().split(".").pop()
    }));

    // ---- 1. Direct tag matches (highest weight: 25 pts each) ----
    let directOverlap = 0;
    for (const target of targetInfo) {
      if (tagSet.has(target.id)) {
        directOverlap += 25;
    ...
    for (const target of targetInfo) {
      ...
      const expansion = this._getBfsExpansion(target.id);

      for (const exp of expansion) {
        let matched = false;
        let matchedTagId = exp.neighborId;

        if (tagSet.has(exp.neighborId)) {
          matched = true;
        } else {
          const suffixMatch = suffixMap.get(exp.neighborSuffix);
          if (suffixMatch) {
            matched = true;
        ...
        if (matched) {
          tagGraphCredit += exp.contributionFactor;
          topContributors.push({
            sourceQueryTagId: target.id,
            targetCourseTagId: matchedTagId,
            path: exp.path,
            contribution: Math.round(exp.contributionFactor * 100) / 100,
          });
        }
      }
