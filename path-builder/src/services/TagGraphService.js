...
class TagGraphService {
  ...
  /**
   * Internal helper to pre-calculate and cache course tag sets and suffixes.
   */
  _getCourseMetadata(course) {
    ...
    const meta = { courseTagSet, suffixMap, allCourseTags };
    this._courseMetadata.set(course, meta);
    return meta;
  }

  /**
   * Internal helper to pre-calculate and cache BFS expansions for a target tag.
   */
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          const creditMultiplier = 5 * dirWeight * hopMultiplier * edgeDataWeight;

          // Reconstruct path for this neighbor
          const path = [];
          let cur = neighborId;
          ...
          expansion.push({
            ...
            creditMultiplier,
            path: [...path], // Clone to prevent mutation
          });
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Use identity-based cache for course metadata
    const { courseTagSet, suffixMap } = this._getCourseMetadata(course);

    // Unique target tag IDs
    ...
    for (const tagId of uniqueTargets) {
      ...
      for (const entry of expansion) {
        ...
        if (matched) {
          tagGraphCredit += entry.creditMultiplier;
          topContributors.push({
            sourceQueryTagId: tagId,
            targetCourseTagId: matchedTagId,
            path: [...entry.path], // Clone path from cache to prevent shared-reference mutation
            contribution: Math.round(entry.creditMultiplier * 100) / 100,
          });
        }
      }
