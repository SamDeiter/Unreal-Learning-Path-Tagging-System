...
class TagGraphService {
  constructor() {
    ...
    // Cache for pre-normalized course tags (used in scoreCourseRelevance)
    this._courseTagCache = new WeakMap();

    // Cache for graph expansion results (used in scoreCourseRelevance)
  ...
  /**
   * Internal: Calculate graph expansion for a tag up to MAX_HOPS.
   * Cached to avoid redundant BFS during batch scoring.
   */
  _calculateTagExpansion(tagId) {
    const MAX_HOPS = 2;
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Check cache for pre-normalized tags
    let cached = this._courseTagCache.get(course);
    if (!cached) {
      ...
      // Pre-calculate suffixes for faster matching
      const courseTagSuffixes = new Map();
      allCourseTags.forEach((t) => {
        const s = t.split(".").pop();
        if (s && s.length > 2) {
          if (!courseTagSuffixes.has(s)) courseTagSuffixes.set(s, t);
        }
      });

      cached = { allCourseTags, courseTagSet, geminiTags, courseTagSuffixes };
      this._courseTagCache.set(course, cached);
    }

    const { _allCourseTags: _, courseTagSet, geminiTags, courseTagSuffixes } = cached;
    ...
    for (const target of targetSet) {
      ...
      if (suffix.length > 2) {
        const matchedFullTag = courseTagSuffixes.get(suffix);
        if (matchedFullTag) {
          directOverlap += 15;
          topContributors.push({
            sourceQueryTagId: target,
    ...
    for (const tagId of targetTagIds) {
      ...
      // Use expansion cache to avoid redundant BFS
      let expansion = this._expansionCache.get(tagId);
      if (!expansion) {
        expansion = this._calculateTagExpansion(tagId);
      ...
      for (const entry of expansion) {
        ...
        if (courseTagSet.has(entry.neighborId)) {
          matched = true;
        } else if (entry.neighborSuffix.length > 2 && courseTagSuffixes.has(entry.neighborSuffix)) {
          matched = true;
        }
