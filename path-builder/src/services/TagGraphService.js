...
class TagGraphService {
  constructor() {
    ...
    // Cache for BFS graph expansions (keyed by tagId)
    this._expansionCache = new Map();

    // Cache for course metadata (normalized tags and suffix map)
  ...
  /**
   * Internal helper: Extracts and normalizes course tags for matching.
   * Cached via WeakMap for performance.
   */
  _getNormalizedCourseMetadata(course) {
    ...
    // Also include legacy tags object
    ...
    // Pre-calculate suffixes for $O(1)$ lookup in score loop
    ...
    const metadata = { tagSet, suffixMap, tags };
    this._courseMetadataCache.set(course, metadata);
    return metadata;
  }

  /**
   * Internal helper: Performs BFS graph expansion for a tag.
  ...
  _getGraphExpansion(tagId) {
    ...
    this._expansionCache.set(tagId, results);
    return results;
  }
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    const { tagSet: courseTagSet, suffixMap: courseSuffixMap } =
      this._getNormalizedCourseMetadata(course);
    ...
    for (const target of targetSet) {
      ...
      if (suffix && suffix.length > 2 && courseSuffixMap.has(suffix)) {
    ...
    for (const tagId of targetTagIds) {
      ...
      for (const exp of expansion) {
        ...
        if (courseTagSet.has(exp.neighborId)) {
          matched = true;
        } else if (exp.neighborSuffix.length > 2 && courseSuffixMap.has(exp.neighborSuffix)) {
          matched = true;
          matchedTagId = courseSuffixMap.get(exp.neighborSuffix);
        }
