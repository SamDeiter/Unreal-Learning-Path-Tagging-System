...
class TagGraphService {
  ...
  /**
   * Internal helper to pre-calculate and cache course metadata for faster matching.
   * Uses identity-based WeakMap to prevent memory leaks and redundant processing.
   */
  _getCourseMetadata(course) {
    ...
    const metadata = { allCourseTags, tagSet, suffixSet };
    this._courseMetadata.set(course, metadata);
    return metadata;
  }

  /**
   * Internal helper to pre-calculate and cache BFS expansion for a tag.
   * Since the graph is static, this can be cached across all courses in a batch.
   */
   ...
   * Uses identity-based metadata caching and pre-calculated BFS expansions.
   *
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // 1. Get cached metadata (identity-based)
    const metadata = this._getCourseMetadata(course);
    const { tagSet, suffixSet, allCourseTags } = metadata;

    // Deduplicate target tags to avoid double-counting
    ...
    for (const targetId of uniqueTargetIds) {
      ...
      if (tagSet.has(target)) {
        ...
        if (suffix && suffix.length > 2 && suffixSet.has(suffix)) {
          directOverlap += 15;
          // Find which course tag actually matched this suffix for the breakdown
          const matchedTag = allCourseTags.find(ct => ct.split(".").pop() === suffix);
          topContributors.push({
            sourceQueryTagId: targetId,
            targetCourseTagId: matchedTag || targetId,
            path: [],
            contribution: 15,
          });
        }
    ...
    if (geminiTags.length > 0) {
      for (const targetId of uniqueTargetIds) {
        const targetSuffix = targetId.toLowerCase().split(".").pop();
    ...
    for (const tagId of uniqueTargetIds) {
      ...
      for (const entry of expansion) {
        ...
        if (tagSet.has(entry.neighborId)) {
          ...
          matched = true;
          matchedTagId = allCourseTags.find(ct => ct.split(".").pop() === entry.neighborSuffix) || matchedTagId;
        }
