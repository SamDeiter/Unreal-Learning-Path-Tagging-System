...
class TagGraphService {
  constructor() {
    ...
    // Legacy support for tests/dependent modules
    this.termIndex = [
      ...this.phraseIndex,
      ...Array.from(this.termMap.values()).flat()
    ];
  ...
  /**
   * V2: Build partitioned term indices for whole-word/phrase matching.
   * Single terms go into a Map for O(1) lookup.
   * Phrases go into an array for O(N) regex matching.
   * @private
   */
  _buildTermIndices() {
    this.phraseIndex = [];
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Use course metadata cache to avoid repeated tag normalization
    ...
    if (!metadata) {
      ...
      metadata = {
        allCourseTags,
        courseTagSet: new Set(allCourseTags),
        geminiTags: (course.gemini_system_tags || []).map((t) => t.toLowerCase()),
      };
      this._courseMetadataCache.set(course, metadata);
    }

    const { allCourseTags, courseTagSet, geminiTags } = metadata;
    ...
    for (const target of targetSet) {
      ...
      for (const ct of allCourseTags) {
        if (ct.endsWith(`.${suffix}`) && suffix.length > 2) {
          directOverlap += 15;
          topContributors.push({
            sourceQueryTagId: target,
    ...
    for (const target of targetSet) {
      ...
      if (
        geminiTags.some(
          (gt) => gt === targetSuffix || gt.includes(targetSuffix)
        )
      ) {
        geminiBonus += 10;
      }
    ...
    for (const tagId of targetTagIds) {
      ...
      for (const exp of expansion) {
        ...
        if (courseTagSet.has(exp.neighborIdLower)) {
          ...
          for (const ct of allCourseTags) {
            if (ct.endsWith(`.${exp.neighborSuffix}`) && exp.neighborSuffix.length > 2) {
              matched = true;
              break;
            }
          }
        ...
        if (matched) {
          const credit = 5 * exp.creditMultiplier;
          tagGraphCredit += credit;
          topContributors.push({
            sourceQueryTagId: tagId,
            targetCourseTagId: exp.neighborId,
            path: exp.path,
  ...
  /**
   * BFS Graph expansion for a specific tag.
   * Caches results to avoid redundant traversals during batch scoring.
   * @private
   */
  _getGraphExpansion(tagId) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          matches.push({
            ...
            neighborIdLower: edge.neighborId.toLowerCase(),
            neighborSuffix: edge.neighborId.split(".").pop(),
            creditMultiplier,
            path,
          });
    ...
    this._expansionCache.set(tagId, matches);
    return matches;
  }
  ...
  extractTagsFromText(text) {
    ...
    // Build set of searchable word forms (original + depluralized)
    const queryWordForms = new Set();
    for (const w of words) {
      queryWordForms.add(w);
      queryWordForms.add(depluralize(w));
    }
    ...
    // 1. Phrase matching (O(N_phrases) regex checks)
    ...
    // 2. Single-term matching (O(N_words) Map lookups)
    for (const wordForm of queryWordForms) {
      const termMatches = this.termMap.get(wordForm);
      if (termMatches) {
