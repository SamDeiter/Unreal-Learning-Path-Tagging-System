...
class TagGraphService {
  constructor() {
    ...
    // V2 Bolt: Initialize caches for expensive course scoring
    this._courseMetadata = new WeakMap(); // course object -> { tagSet, suffixes }
    this._bfsCache = new Map(); // tagId -> { credits: Map<neighborId, credit>, paths: Map<neighborId, path> }
  ...
  /**
   * V2 Bolt: Get pre-calculated course metadata (tags and suffixes)
   * Uses WeakMap to cache based on course object identity.
   */
  _getCourseMetadata(course) {
    ...
    const metadata = {
      allTags: allCourseTags,
      tagSet: new Set(allCourseTags),
      suffixes: allCourseTags.map((t) => t.split(".").pop()).filter((s) => s.length > 2),
    };
  ...
  /**
   * V2 Bolt: Get pre-calculated BFS expansion for a tag.
   * Uses Map to cache results for the query session.
   */
  _getBfsExpansion(tagId) {
    ...
    const results = {
      matches: new Map(), // neighborId -> { credit, path }
    };
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          results.matches.set(edge.neighborId, {
            credit,
            path,
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // V2 Bolt: Use cached metadata for course tags
    const { allTags: allCourseTags, tagSet: courseTagSet, suffixes: courseSuffixes } = this._getCourseMetadata(course);
    ...
    for (const target of targetSet) {
      ...
      if (targetSuffix.length > 2 && courseSuffixes.includes(targetSuffix)) {
        directOverlap += 15;
        // Find which tag actually matched for the contributor list
        const matchedTag = allCourseTags.find((ct) => ct.endsWith("." + targetSuffix) || ct === targetSuffix);
        topContributors.push({
          sourceQueryTagId: target,
    ...
    for (const tagId of targetTagIds) {
      ...
      const expansion = this._getBfsExpansion(tagId.toLowerCase());

      for (const [neighborId, { credit, path, suffix }] of expansion.matches) {
        let matched = false;
        if (courseTagSet.has(neighborId)) {
          matched = true;
        } else if (suffix && suffix.length > 2 && courseSuffixes.includes(suffix)) {
          matched = true;
        }
        ...
        if (matched) {
          ...
          topContributors.push({
            ...
            targetCourseTagId: neighborId,
            path: [...path], // Clone to prevent shared-reference mutations
