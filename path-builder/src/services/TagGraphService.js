...
class TagGraphService {
  constructor() {
    ...
    // Cache for course metadata (normalized tags and suffix map) to avoid redundant processing
    this._courseMetadataCache = new WeakMap();

    // Cache for graph expansion (BFS) results for target tags
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // 0. Use cache for course metadata to avoid redundant processing in hot loops
    ...
    if (!metadata) {
      ...
      const courseTagSet = new Set(tags);
      const suffixMap = new Map();
      for (const ct of tags) {
        const parts = ct.split(".");
        const suffix = parts[parts.length - 1];
        if (suffix && suffix.length > 2) {
          suffixMap.set(suffix, ct);
        }
      }
    ...
    const { courseTagSet, suffixMap, geminiTags } = metadata;
    ...
    for (const target of targetSet) {
      ...
      // Check suffix match: "lumen" matches course tag "rendering.lumen" using O(1) lookup
      const targetParts = target.split(".");
      const suffix = targetParts[targetParts.length - 1];
    ...
    for (const target of targetSet) {
      const targetParts = target.split(".");
      const targetSuffix = targetParts[targetParts.length - 1];
      if (
        geminiTags.some(
    ...
    for (const tagId of targetTagIds) {
      // Use expansion cache to avoid re-running BFS for every course in a search batch
        ...
        while (frontier.length > 0) {
          ...
          for (const { id, hops } of frontier) {
            ...
            for (const edge of [...outgoing, ...incoming]) {
              ...
              // Path reconstruction (pre-calculate for the expansion result)
              const path = [];
              ...
              const neighborParts = neighborTag.split(".");
              const neighborSuffix = neighborParts[neighborParts.length - 1];

              expansion.push({
                tagId: neighborTag,
                suffix: neighborSuffix,
                credit,
                path
              });
      ...
      for (const node of expansion) {
        let matched = false;
        if (courseTagSet.has(node.tagId)) {
          matched = true;
        } else if (node.suffix && node.suffix.length > 2 && suffixMap.has(node.suffix)) {
          matched = true;
        }
