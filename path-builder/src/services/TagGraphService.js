...
class TagGraphService {
  constructor() {
    ...
    this._courseMetaCache = new WeakMap();
  ...
  /**
   * Internal: Pre-process course tags into a cached metadata object.
   * Reduces redundant string operations and Set creation in batch scoring.
   */
  _getCourseMetadata(course) {
    if (this._courseMetaCache.has(course)) {
      return this._courseMetaCache.get(course);
    }
    ...
    if (course.tags && typeof course.tags === "object" && !Array.isArray(course.tags)) {
      Object.values(course.tags).forEach((v) => {
        if (typeof v === "string") rawTags.push(v);
      });
    }
    ...
    const meta = { tagSet, suffixMap, geminiTags, allTags };
    this._courseMetaCache.set(course, meta);
    return meta;
  }

  /**
   * Internal: Pre-calculate graph expansion for a tag ID with caching.
   * Reduces $O(N * G)$ graph traversals to $O(G)$ during batch scoring.
   */
  _getExpansion(tagId) {
    ...
    const reachable = new Map(); // neighborId -> { credit, path }
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          const neighborLower = edge.neighborId.toLowerCase();
          reachable.set(neighborLower, {
            credit,
            path,
            suffix: neighborLower.split(".").pop(),
          });
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    for (const tagId of targetTagIds) {
      ...
      const expansion = this._getExpansion(tagId);

      for (const [neighborId, { credit, path, suffix }] of expansion) {
        let matched = false;
        let matchedTagId = neighborId;

        if (tagSet.has(neighborId)) {
          matched = true;
        } else if (suffix && suffix.length > 2 && suffixMap.has(suffix)) {
          matched = true;
          matchedTagId = suffixMap.get(suffix);
        }
