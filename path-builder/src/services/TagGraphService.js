...
class TagGraphService {
  constructor() {
    ...
    // Initialize caches for high-frequency operations
    this._relatedCache = new Map();
    this._courseMetadataCache = new WeakMap(); // Identity-based course data cache
    this._expansionCache = new Map(); // BFS graph expansion cache
    
    // Cache for query-level normalization to avoid re-processing in batch loops
    this._lastTargetTagIds = null;
    this._targetTagMetas = null;
  ...
  /**
   * Get pre-normalized course metadata from WeakMap cache
   * @private
   */
  _getCourseMetadata(course) {
    ...
    // Pre-calculate suffix map for O(1) matching against query suffixes
    const suffixMap = new Map();
    for (const ct of allCourseTags) {
      const ctSuffix = ct.split(".").pop();
      if (ctSuffix && ctSuffix.length > 2) {
        suffixMap.set(ctSuffix, ct);
      }
    }

    const metadata = { allCourseTags, courseTagSet, geminiTags, suffixMap };
    this._courseMetadataCache.set(course, metadata);
    return metadata;
  }

  /**
   * Get BFS graph expansion for a tag ID from cache or compute it
   * @private
   */
  _getExpansion(tagId) {
    ...
    // Track parent references for lazy path reconstruction
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          const neighborSuffix = edge.neighborId.toLowerCase().split(".").pop();
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Cache query-level data if targetTagIds changed since last call
    if (this._lastTargetTagIds !== targetTagIds) {
      this._lastTargetTagIds = targetTagIds;
      const uniqueTargets = [...new Set(targetTagIds.map(t => t.toLowerCase()))];
      this._targetTagMetas = uniqueTargets.map(id => ({
        id,
        suffix: id.split(".").pop()
      }));
    }

    const { courseTagSet, geminiTags, suffixMap } = this._getCourseMetadata(course);
    const topContributors = [];

    // ---- 1. Direct tag matches (highest weight: 25 pts each) ----
    let directOverlap = 0;
    for (const target of this._targetTagMetas) {
      // Exact match on full tag ID (O(1) with Set)
      if (courseTagSet.has(target.id)) {
        directOverlap += 25;
        topContributors.push({
          sourceQueryTagId: target.id,
          targetCourseTagId: target.id,
          path: [],
          contribution: 25,
        });
        continue;
      ...
      // Suffix match: "lumen" matches course tag "rendering.lumen" (O(1) with suffixMap)
      if (target.suffix.length > 2 && suffixMap.has(target.suffix)) {
        const matchedCourseTag = suffixMap.get(target.suffix);
        directOverlap += 15;
        topContributors.push({
          sourceQueryTagId: target.id,
          targetCourseTagId: matchedCourseTag,
          path: [],
          contribution: 15,
        });
      }
    ...
    let geminiBonus = 0;
    for (const target of this._targetTagMetas) {
      if (geminiTags.some(gt => gt === target.suffix || gt.includes(target.suffix))) {
        geminiBonus += 10;
      }
    }
    ...
    for (const target of this._targetTagMetas) {
      ...
      const expansion = this._getExpansion(target.id);

      for (const { neighborId, neighborSuffix, creditMultiplier, path } of expansion) {
        let matched = false;
        let matchedCourseTagId = neighborId;

        if (courseTagSet.has(neighborId)) {
          matched = true;
        } else if (neighborSuffix.length > 2 && suffixMap.has(neighborSuffix)) {
          matched = true;
          matchedCourseTagId = suffixMap.get(neighborSuffix);
        }

        if (matched) {
          const credit = 5 * creditMultiplier;
          tagGraphCredit += credit;
          topContributors.push({
            sourceQueryTagId: target.id,
            targetCourseTagId: matchedCourseTagId,
            path,
