...
class TagGraphService {
   ...
   * Prevents re-normalizing tags and suffixes across multiple score calls.
   */
  ...
  /**
   * Internal: Get or build cached BFS expansion for a tag.
   * Prevents redundant graph traversal when scoring thousands of courses.
   */
  _getBfsExpansion(tagId) {
    const normalizedTagId = tagId.toLowerCase();
    if (this._bfsExpansionCache.has(normalizedTagId)) {
      return this._bfsExpansionCache.get(normalizedTagId);
    }
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    const { courseTagSet, suffixMap, geminiTags } = this._getCourseMetadata(course);
    ...
    for (const target of targetSet) {
      ...
      if (suffix && suffix.length > 2 && suffixMap.has(suffix)) {
        ...
        topContributors.push({
          sourceQueryTagId: target,
          targetCourseTagId: suffixMap.get(suffix)[0],
          path: [],
          contribution: 15,
        });
      }
    ...
    for (const tagId of targetTagIds) {
      ...
      for (const edge of expansion) {
        let matched = false;
        let matchedTagId = edge.neighborId;

        if (courseTagSet.has(edge.neighborId)) {
          matched = true;
        } else if (edge.neighborSuffix.length > 2 && suffixMap.has(edge.neighborSuffix)) {
          matched = true;
          matchedTagId = suffixMap.get(edge.neighborSuffix)[0];
        }

        if (matched) {
          tagGraphCredit += edge.creditBase;
          topContributors.push({
            sourceQueryTagId: tagId,
            targetCourseTagId: matchedTagId,
            path: edge.path,
            contribution: Math.round(edge.creditBase * 100) / 100,
          });
        }
      }
