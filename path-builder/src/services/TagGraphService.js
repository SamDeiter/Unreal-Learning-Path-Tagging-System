...
class TagGraphService {
  ...
  /**
   * Internal helper to pre-calculate and cache course metadata for relevance scoring.
   * @param {Object} course
   * @returns {{tagSet: Set<string>, tags: string[], suffixes: string[]}}
   */
  _getCourseMetadata(course) {
    ...
    const metadata = {
      ...
      tags: allCourseTags,
      suffixes: allCourseTags.map((t) => t.split(".").pop()),
    };
   ...
   * Optimized with identity-based caching.
   *
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    const targetIdsLower = [...new Set(targetTagIds.map((t) => t.toLowerCase()))];
    const courseMeta = this._getCourseMetadata(course);
    const topContributors = [];

    // ---- 1. Direct tag matches (highest weight: 25 pts each) ----
    let directOverlap = 0;
    for (const target of targetIdsLower) {
      if (courseMeta.tagSet.has(target)) {
        directOverlap += 25;
      ...
      if (suffix.length > 2) {
        const matchingTagIndex = courseMeta.suffixes.indexOf(suffix);
        if (matchingTagIndex !== -1) {
          const ct = courseMeta.tags[matchingTagIndex];
          directOverlap += 15;
          topContributors.push({
            sourceQueryTagId: target,
    ...
    for (const tagId of targetIdsLower) {
      let tagGraphCredit = 0;
      const expansions = this._getBfsExpansion(tagId);

      for (const exp of expansions) {
        let matched = false;
        if (courseMeta.tagSet.has(exp.neighborId)) {
          matched = true;
        } else if (exp.neighborSuffix.length > 2 && courseMeta.suffixes.includes(exp.neighborSuffix)) {
          matched = true;
        }

        if (matched) {
          tagGraphCredit += exp.creditBase;
          topContributors.push({
            sourceQueryTagId: tagId,
            targetCourseTagId: exp.neighborId,
            path: exp.path,
            contribution: Math.round(exp.creditBase * 100) / 100,
          });
        }
      }
