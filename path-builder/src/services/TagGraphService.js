...
class TagGraphService {
  constructor() {
    ...
    /**
     * V2 Performance Caches:
     * - _courseMetadata: WeakMap for identity-based course tag normalization
     * - _bfsCache: Map for query-tag graph expansion results (cleared between batches)
     */
  ...
  /**
   * Helper: Get pre-calculated metadata for a course.
   * Caches results using a WeakMap to avoid redundant normalization.
   * @private
   */
  _getCourseMetadata(course) {
    ...
    allTags.forEach(t => {
      const s = t.split('.').pop();
      if (s && s.length > 2) suffixSet.add(s);
    });

    const metadata = { allTags, tagSet, suffixSet };
    this._courseMetadata.set(course, metadata);
    return metadata;
  }

  /**
   * Helper: Get pre-calculated 2-hop BFS results for a tag.
   * Caches results in a Map (cleared between search batches).
   * @private
   */
   ...
   * Optimized with identity-based metadata caching and BFS memoization.
   *
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Deduplicate target tags to avoid redundant processing and score inflation
    const uniqueTargetIds = [...new Set(targetTagIds)];
    const courseMeta = this._getCourseMetadata(course);
    const topContributors = [];

    // ---- 1. Direct tag matches (highest weight: 25 pts each) ----
    let directOverlap = 0;
    for (const target of uniqueTargetIds) {
      const targetLower = target.toLowerCase();

      // Exact match on full tag ID
      if (courseMeta.tagSet.has(targetLower)) {
        directOverlap += 25;
      ...
      const suffix = targetLower.split(".").pop();
      if (suffix && suffix.length > 2 && courseMeta.suffixSet.has(suffix)) {
        // Find WHICH course tag matched for the contributor list
        const matchingTag = courseMeta.allTags.find(ct => ct.endsWith(`.${suffix}`) || ct === suffix);
        directOverlap += 15;
        topContributors.push({
          sourceQueryTagId: target,
          targetCourseTagId: matchingTag || targetLower,
          path: [],
          contribution: 15,
        });
      }
    ...
    for (const tagId of uniqueTargetIds) {
      let tagGraphCredit = 0;
      const expansion = this._getBfsExpansion(tagId);

      for (const entry of expansion) {
        let matched = false;
        if (courseMeta.tagSet.has(entry.neighborId.toLowerCase())) {
          matched = true;
        } else if (entry.neighborSuffix.length > 2 && courseMeta.suffixSet.has(entry.neighborSuffix)) {
          matched = true;
        }
        ...
        if (matched) {
          ...
          topContributors.push({
            ...
            path: [...entry.path], // Clone path to prevent mutation
  ...
  /**
   * Clear the related-tag cache and BFS cache. Call between query batches.
   */
  clearRelatedCache() {
