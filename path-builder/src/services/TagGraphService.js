...
class TagGraphService {
  ...
  /**
   * Internal helper to pre-calculate and cache BFS expansions for target tags.
   * This avoids redundant O(G_local) graph traversals during batch course scoring.
   */
  _getBfsExpansion(tagId) {
    if (this._bfsExpansionCache.has(tagId)) return this._bfsExpansionCache.get(tagId);

    const neighbors = [];
    const MAX_HOPS = 2;
    const HOP_ATTENUATION = 0.5;
    const startId = tagId.toLowerCase();
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          const potentialCredit = 5 * dirWeight * hopMultiplier * (edge.weight || 0.5);
    ...
    this._bfsExpansionCache.set(tagId, neighbors);
    return neighbors;
  }

  /**
   * Internal helper to pre-calculate and cache course tag metadata.
   * Flattens multi-source tags and pre-computes suffixes to avoid O(T) inner loop ops.
   */
  _getCourseMetadata(course) {
    ...
    const suffixSet = new Set(tags.map((t) => t.split(".").pop()));
    const geminiTags = (course.gemini_system_tags || []).map((t) => t.toLowerCase());

    const metadata = { tagSet, suffixSet, geminiTags, tags };
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    const { tagSet: courseTagSet, suffixSet: courseSuffixSet, geminiTags, tags: allCourseTags } = metadata;
    ...
    for (const target of targetSet) {
      ...
      if (courseSuffixSet.has(suffix) && suffix.length > 2) {
        ...
        topContributors.push({
          sourceQueryTagId: target,
          targetCourseTagId: matchedTag || suffix,
          path: [],
          contribution: 15,
        });
        continue;
    ...
    for (const tagId of targetTagIds) {
      ...
      const neighbors = this._getBfsExpansion(tagId);

      for (const neighbor of neighbors) {
        let matched = false;
        if (courseTagSet.has(neighbor.id)) {
          matched = true;
        } else if (neighbor.suffix.length > 2 && courseSuffixSet.has(neighbor.suffix)) {
          matched = true;
        }
