...
class TagGraphService {
  constructor() {
    ...
    // Cache for course metadata (identity-based)
  ...
  /**
   * Internal helper to get/cache course metadata for scoring
   * @private
   */
  _getCourseMetadata(course) {
    ...
    // Pre-calculate suffixes for O(1) matching
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Reuse or normalize target tag IDs
    if (this._lastTargetTagIds !== targetTagIds) {
      this._normalizedTargetIds = targetTagIds.map(t => t.toLowerCase());
      this._lastTargetTagIds = targetTagIds;
    }
    const targetIds = this._normalizedTargetIds;
    ...
    for (const tagId of targetIds) {
      ...
      // Get or compute graph expansion for this tag
      ...
      // Score against expansion
      for (const entry of expansion) {
        let matched = false;
        if (courseTagSet.has(entry.id)) {
          matched = true;
        } else if (entry.suffix && entry.suffix.length > 2 && suffixMap.has(entry.suffix)) {
          matched = true;
        }
