...
class TagGraphService {
  constructor() {
    ...
    // V3 Performance: Identity-based caching for course metadata and graph expansions
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // V3 Performance: Identity-based caching for course metadata (normalized tags and suffixes)
    ...
    const { allCourseTags, courseTagSet, suffixSet } = metadata;
    ...
    for (const target of targetSet) {
      ...
      if (suffix.length > 2 && suffixSet.has(suffix)) {
        directOverlap += 15;
        // Note: For topContributors, we still need a representative full tag ID from the course
        const representativeTag = allCourseTags.find((ct) => ct.endsWith(`.${suffix}`) || ct === suffix) || target;
        topContributors.push({
          sourceQueryTagId: target,
          targetCourseTagId: representativeTag,
          path: [],
          contribution: 15,
        });
      }
    ...
    for (const tagId of targetTagIds) {
      // V3 Performance: Identity-based caching for graph expansion (BFS)
      ...
      for (const entry of expansion) {
        if (courseTagSet.has(entry.tagId) || (entry.suffix.length > 2 && suffixSet.has(entry.suffix))) {
