...
class TagGraphService {
  ...
  /**
   * Helper to get or build cached course metadata (normalized tags, suffixes)
   * @private
   */
  _getCourseMetadata(course) {
    ...
    const courseSuffixMap = new Map();
    for (const ct of allCourseTags) {
      const suffix = ct.split(".").pop();
      if (suffix && suffix.length > 2) {
        courseSuffixMap.set(suffix, ct);
      }
    }
  ...
  /**
   * Helper to get or build cached BFS expansion for a tag
   * @private
   */
  _getBfsExpansion(tagId) {
    const normalizedTagId = tagId.toLowerCase();
    if (this._bfsExpansionCache.has(normalizedTagId)) return this._bfsExpansionCache.get(normalizedTagId);

    const MAX_HOPS = 2;
    const HOP_ATTENUATION = 0.5;

    const expansion = []; // Array of { neighborId, neighborSuffix, credit, path }
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          const neighborIdNormalized = edge.neighborId.toLowerCase();
          if (visited.has(neighborIdNormalized)) continue;
          visited.add(neighborIdNormalized);

          parentMap.set(neighborIdNormalized, { parentId: id, edgeType: edge.relation });
          ...
          const creditBase = 5 * dirWeight * hopMultiplier * edgeDataWeight;

          const path = [];
          let cur = neighborIdNormalized;
          ...
          expansion.push({
            neighborId: neighborIdNormalized,
            neighborSuffix: neighborIdNormalized.split(".").pop(),
            credit: creditBase,
            path: path,
          });

          nextFrontier.push({ id: neighborIdNormalized, hops: hops + 1 });
        }
      }
      frontier = nextFrontier;
    }

    this._bfsExpansionCache.set(normalizedTagId, expansion);
    return expansion;
  }
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Use cached course metadata
    const { courseTagSet, courseSuffixMap, geminiTags } = this._getCourseMetadata(course);
    const topContributors = [];

    // Deduplicate target tags for scoring efficiency
    const targetTagIdsNormalized = [...new Set(targetTagIds.map(t => t.toLowerCase()))];

    // ---- 1. Direct tag matches (highest weight: 25 pts each) ----
    let directOverlap = 0;
    for (const target of targetTagIdsNormalized) {
      ...
      if (suffix.length > 2 && courseSuffixMap.has(suffix)) {
        const matchedCourseTag = courseSuffixMap.get(suffix);
        directOverlap += 15;
        topContributors.push({
          sourceQueryTagId: target,
    ...
    for (const tagId of targetTagIdsNormalized) {
      let tagGraphCredit = 0;
      const expansion = this._getBfsExpansion(tagId);

      for (const entry of expansion) {
        let matched = false;
        let matchedId = entry.neighborId;

        if (courseTagSet.has(entry.neighborId)) {
          matched = true;
        } else if (entry.neighborSuffix.length > 2 && courseSuffixMap.has(entry.neighborSuffix)) {
          matched = true;
          matchedId = courseSuffixMap.get(entry.neighborSuffix);
        }
