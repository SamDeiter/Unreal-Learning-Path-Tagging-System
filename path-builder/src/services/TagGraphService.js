...
class TagGraphService {
  constructor() {
    ...
    this._normalizedTargetTagIds = null;
   ...
   * Uses identity-based caching, BFS expansion caching, and edge-type weights.
   *
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // 1. Identity-based course metadata caching
    let courseMeta = this._courseTagCache.get(course);
    if (!courseMeta) {
      const allCourseTags = [
      ...
      allCourseTags.forEach(t => {
        const s = t.split(".").pop();
        if (s && s.length > 2) suffixMap.set(s, t);
      });

      courseMeta = { tagSet, suffixMap, geminiTags: (course.gemini_system_tags || []).map(t => t.toLowerCase()) };
      this._courseTagCache.set(course, courseMeta);
    }

    const { tagSet: courseTagSet, suffixMap: courseSuffixMap, geminiTags } = courseMeta;

    // 2. Query normalization caching
    if (this._lastTargetTagIds !== targetTagIds) {
      this._normalizedTargetTagIds = [...new Set(targetTagIds.map(t => t.toLowerCase()))];
      this._lastTargetTagIds = targetTagIds;
    }
    const normalizedTargets = this._normalizedTargetTagIds;
    ...
    for (const target of normalizedTargets) {
      ...
      if (courseSuffixMap.has(suffix)) {
        directOverlap += 15;
        topContributors.push({ sourceQueryTagId: target, targetCourseTagId: courseSuffixMap.get(suffix), path: [], contribution: 15 });
      }
    }
    ...
    for (const tagId of normalizedTargets) {
      // 3a. Use cached BFS expansion for this query tag
      let expansion = this._expansionCache.get(tagId);
      if (!expansion) {
        expansion = this._calculateExpansion(tagId, MAX_HOPS, HOP_ATTENUATION);
        this._expansionCache.set(tagId, expansion);
      }

      let tagGraphCredit = 0;
      for (const entry of expansion) {
        let matched = courseTagSet.has(entry.id) || courseSuffixMap.has(entry.suffix);

        if (matched) {
          const actualTargetCourseTag = courseTagSet.has(entry.id) ? entry.id : courseSuffixMap.get(entry.suffix);
          tagGraphCredit += entry.credit;
          topContributors.push({
            sourceQueryTagId: tagId,
            targetCourseTagId: actualTargetCourseTag,
            path: entry.path,
  ...
  /**
   * Internal: Calculate graph expansion for a tag (BFS)
   */
  _calculateExpansion(tagId, maxHops, attenuation) {
    const expansion = [];
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        const outgoing = (this.edgesBySource.get(id) || []).map(e => ({ ...e, direction: "forward", neighborId: e.target }));
        const incoming = (this.edgesByTarget.get(id) || []).map(e => ({ ...e, direction: "reverse", neighborId: e.source }));
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          const credit = 5 * dirWeight * Math.pow(attenuation, hops) * (edge.weight || 0.5);
          ...
          expansion.push({
            id: edge.neighborId,
            suffix: edge.neighborId.split(".").pop(),
            credit,
            path
          });
  ...
  /**
   * Clear caches. Call between query batches.
   */
  clearRelatedCache() {
