...
class TagGraphService {
  constructor() {
    ...
    this._bfsCache = new Map(); // tagId -> Array of { tagId, suffix, creditBase, path }
    this._courseMetadata = new WeakMap(); // course -> { tagSet, suffixSet, geminiTags }
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Use pre-calculated course metadata
    ...
    for (const target of targetSet) {
      ...
      if (suffix && suffix.length > 2 && suffixSet.has(suffix)) {
        directOverlap += 15;
        // Find the actual course tag that matched this suffix for the breakdown
        let actualMatchedTag = target;
        for (const ct of tagSet) {
          if (ct.endsWith("." + suffix) || ct === suffix) {
            actualMatchedTag = ct;
            break;
          }
        }
        topContributors.push({
          sourceQueryTagId: target,
          targetCourseTagId: actualMatchedTag,
          path: [],
          contribution: 15,
        });
      }
    ...
    for (const tagId of targetTagIds) {
      ...
      // Use pre-calculated graph expansion
      const expansion = this._getBfsExpansion(tagId);

      for (const entry of expansion) {
        let matched = false;
        if (tagSet.has(entry.tagId)) {
          matched = true;
        } else if (entry.suffix && entry.suffix.length > 2 && suffixSet.has(entry.suffix)) {
          matched = true;
        }

        if (matched) {
          const credit = 5 * entry.creditBase;
          tagGraphCredit += credit;
          topContributors.push({
            sourceQueryTagId: tagId,
            targetCourseTagId: entry.tagId,
            path: [...entry.path], // Clone to prevent mutation
  ...
  /**
   * Internal: Get pre-calculated course metadata (tags, suffixes)
   * @param {Object} course
   * @returns {Object} { tagSet, suffixSet, geminiTags }
   */
  _getCourseMetadata(course) {
    ...
    for (const ct of allCourseTags) {
      const s = ct.split(".").pop();
      if (s && s.length > 2) suffixSet.add(s);
    }

    const geminiTags = (course.gemini_system_tags || []).map((t) => t.toLowerCase());

    const metadata = { tagSet, suffixSet, geminiTags };
    this._courseMetadata.set(course, metadata);
    return metadata;
  }

  /**
   * Internal: Get 2-hop graph expansion for a tag
   * @param {string} tagId
   * @returns {Array} List of { tagId, suffix, creditBase, path }
   */
  _getBfsExpansion(tagId) {
    const normalizedTagId = tagId.toLowerCase();
    if (this._bfsCache.has(normalizedTagId)) {
      return this._bfsCache.get(normalizedTagId);
    }
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          const creditBase = dirWeight * hopMultiplier * edgeDataWeight;
