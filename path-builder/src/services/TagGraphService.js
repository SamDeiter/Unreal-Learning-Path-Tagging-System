...
class TagGraphService {
  ...
  /**
   * Get pre-processed course metadata (tags, suffixes) with caching.
   * @private
   */
  ...
  /**
   * Get 2-hop BFS expansion for a tag with caching.
   * @private
   */
  _getBfsExpansion(tagId) {
    ...
    // We use the original case for tagMap/edge lookups, but assume canonical lower for graph
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    for (const target of targetSet) {
      if (tagSet.has(target)) {
        ...
        if (suffix.length > 2 && suffixSet.has(suffix)) {
          ...
          topContributors.push({
            sourceQueryTagId: target,
            targetCourseTagId: `(suffix match: ${suffix})`, // Simplified contributor ID for suffix matches
            path: [],
            contribution: 15,
          });
        }
    ...
    for (const tagId of targetTagIds) {
      ...
      for (const neighbor of expansion) {
        let matched = false;
        if (tagSet.has(neighbor.id)) {
          matched = true;
        } else if (neighbor.suffix.length > 2 && suffixSet.has(neighbor.suffix)) {
          matched = true;
        }

        if (matched) {
          const credit = 5 * neighbor.creditFactor;
