...
class TagGraphService {
  constructor() {
    ...
    // V2 Optimization: Cache graph expansions and course metadata
    ...
    // 4. Build lookup maps for O(1) access (case-insensitive keys)
    this.tagMap = new Map(this.tags.map((t) => [t.tag_id.toLowerCase(), t]));
  ...
  getTag(tagId) {
    ...
    return this.tagMap.get(tagId.toLowerCase()) || null;
  }
  ...
  /**
   * Pre-calculate normalized metadata for a course to optimize batch searches.
   * @param {Object} course
   * @returns {Object} { tagSet: Set, suffixes: Set, geminiTags: string[] }
   */
  _getCourseMetadata(course) {
    ...
    for (const t of allCourseTags) {
      const s = t.split(".").pop();
      if (s && s.length > 2) suffixes.add(s);
    }
  ...
  /**
   * Pre-calculate 2-hop graph expansion for a tag.
   * @param {string} tagId
   * @returns {Array<{neighborId: string, creditBase: number, path: Array}>}
   */
   ...
   * Uses identity-based caching and pre-calculated graph expansions.
   *
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    const { tagSet, suffixes, geminiTags } = this._getCourseMetadata(course);
    ...
    for (const target of uniqueTargets) {
      ...
      if (targetSuffix && targetSuffix.length > 2 && suffixes.has(targetSuffix)) {
        directOverlap += 15;
        // Find WHICH tag matched (for transparency/topContributors)
        for (const ct of tagSet) {
          if (ct.endsWith(`.${targetSuffix}`)) {
            topContributors.push({
              sourceQueryTagId: target,
    ...
    for (const tagId of uniqueTargets) {
      let tagGraphCredit = 0;
      const expansion = this._getBfsExpansion(tagId);

      for (const { neighborId, neighborSuffix, creditBase, path } of expansion) {
        let matched = false;
        if (tagSet.has(neighborId)) {
          matched = true;
        } else if (neighborSuffix.length > 2 && suffixes.has(neighborSuffix)) {
          matched = true;
        }
        ...
        if (matched) {
          ...
          topContributors.push({
            ...
            path: [...path], // Clone to prevent shared reference mutation
            contribution: Math.round(creditBase * 100) / 100,
          });
        }
      }
