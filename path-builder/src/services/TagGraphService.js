...
class TagGraphService {
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Use cache for course metadata to avoid redundant normalization and Set creation
    ...
    for (const target of targetSet) {
      ...
      // Optimization: Using already lower-cased geminiTags
      if (
        geminiTags.some(
    ...
    for (const tagId of targetTagIds) {
      ...
      for (const entry of expansion) {
        ...
        if (courseTagSet.has(entry.neighborIdLower)) {
          ...
          for (const ct of allCourseTags) {
            if (ct.split(".").pop() === entry.neighborSuffix && entry.neighborSuffix.length > 2) {
              matched = true;
              break;
            }
          }
        ...
        if (matched) {
          const credit = 5 * entry.dirWeight * entry.hopMultiplier * entry.edgeDataWeight;
  ...
  /**
   * Internal: Get BFS graph expansion for a tag ID with caching.
   * @param {string} tagId
   * @returns {Array} List of reached nodes and traversal data
   */
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          expansion.push({
            neighborId: edge.neighborId, // Preserve original casing
            neighborIdLower,
            neighborSuffix,
            dirWeight,
            hopMultiplier,
