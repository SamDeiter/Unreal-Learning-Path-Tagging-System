...
class TagGraphService {
  constructor() {
    ...
    // V2 Optimization: Cache for batch operations
  ...
  /**
   * Internal helper to pre-calculate and cache course metadata for batch scoring.
   * @param {Object} course
   * @returns {Object} { tagSet, suffixSet, geminiTags, allCourseTags }
   */
  _getCourseMetadata(course) {
    ...
    const meta = { tagSet, suffixSet, geminiTags, allCourseTags };
    this._courseMetadata.set(course, meta);
    return meta;
  }

  /**
   * Internal helper to pre-calculate and cache BFS expansion for query tags.
   * @param {string} tagId
   * @returns {Array} List of reachable tags with contribution factors
   */
  _getBfsExpansion(tagId) {
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          const contributionFactor = dirWeight * hopMultiplier * edgeDataWeight;
    ...
    this._bfsCache.set(tagId, expansion);
    return expansion;
  }
   ...
   * Optimized with identity-based metadata caching and BFS expansion caching.
   *
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    const { tagSet, suffixSet, geminiTags, allCourseTags } = this._getCourseMetadata(course);

    // Deduplicate target tags to prevent over-scoring
    const targetTagIdsLowerSet = new Set(targetTagIds.map((t) => t.toLowerCase()));
    const topContributors = [];

    // ---- 1. Direct tag matches (highest weight: 25 pts each) ----
    let directOverlap = 0;
    for (const target of targetTagIdsLowerSet) {
      ...
      // Check suffix match (O(1))
      const suffix = target.split(".").pop();
      if (suffixSet.has(suffix)) {
        directOverlap += 15;
        // Find actual matching course tag for traceability
        const matchedTag = allCourseTags.find((ct) => ct === suffix || ct.endsWith("." + suffix));
        topContributors.push({
          sourceQueryTagId: target,
    ...
    for (const tagId of targetTagIdsLowerSet) {
      ...
      for (const entry of expansion) {
        ...
        if (matched) {
          const credit = 5 * entry.contributionFactor;
