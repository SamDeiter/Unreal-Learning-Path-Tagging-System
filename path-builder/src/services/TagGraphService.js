...
class TagGraphService {
  constructor() {
    ...
    this._bfsCache = new Map(); // Cache for 2-hop graph expansions
    this._courseMetadata = new WeakMap(); // Identity-based cache for course tags
  ...
  /**
   * Internal: Get pre-calculated metadata for a course.
   * Caches normalized tag sets and suffixes to avoid redundant O(N) work.
   */
  _getCourseMetadata(course) {
    ...
    for (const t of tagSet) {
      ...
      if (suffix.length > 2) suffixSet.add(suffix);
    }
  ...
  /**
   * Internal: Get 2-hop graph expansion for a tag ID.
   * Caches results to avoid redundant BFS traversals during batch scoring.
   */
  _getBfsExpansion(tagId) {
    ...
    this._bfsCache.set(normalizedTagId, expansion);
    return expansion;
  }

  /**
   * V2: Score a course's relevance for a set of tags.
   * Uses identity-based caching (WeakMap) and BFS expansion caching (Map)
   * to optimize performance for large batches of courses.
   *
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    const { allTags, tagSet, suffixSet } = metadata;
    ...
    for (const target of targetSet) {
      ...
      // O(1) suffix lookup vs O(N) iteration
      const suffix = target.split(".").pop();
      if (suffix.length > 2 && suffixSet.has(suffix)) {
        directOverlap += 15;
        // Find WHICH course tag matched for the contributor list
        const matchedTag = allTags.find(t => t.endsWith("." + suffix) || t === suffix);
        topContributors.push({
          sourceQueryTagId: target,
          targetCourseTagId: matchedTag || target,
          path: [],
          contribution: 15,
        });
      }
    ...
    let geminiBonus = 0;
    const geminiTags = (course.gemini_system_tags || []).map((t) => (typeof t === "string" ? t.toLowerCase() : ""));
    for (const target of targetSet) {
    ...
    for (const tagId of targetTagIds) {
      ...
      for (const node of expansion) {
        let matched = false;
        if (tagSet.has(node.neighborId)) {
          matched = true;
        } else if (node.neighborSuffix.length > 2 && suffixSet.has(node.neighborSuffix)) {
          matched = true;
        }

        if (matched) {
          tagGraphCredit += node.creditBase;
          topContributors.push({
            sourceQueryTagId: tagId,
            targetCourseTagId: node.neighborId,
            path: node.path,
            contribution: Math.round(node.creditBase * 100) / 100,
          });
        }
      }
