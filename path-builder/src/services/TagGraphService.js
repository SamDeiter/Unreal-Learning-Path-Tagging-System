...
class TagGraphService {
  constructor() {
    ...
    // Cache for graph expansions (Map<tagId, Array<{neighborId, weight, relation, hops, path}>>)
    this._bfsCache = new Map();
    // Cache for course tag sets and suffixes (WeakMap<course, {tagSet, suffixes}>)
  ...
  /**
   * Internal helper to pre-calculate or retrieve course metadata (tag sets/suffixes)
   * @param {Object} course
   * @returns {{tagSet: Set<string>, suffixes: Set<string>, geminiTags: string[]}}
   */
    ...
    while (frontier.length > 0) {
      ...
      for (const { id, hops } of frontier) {
        ...
        for (const edge of [...outgoing, ...incoming]) {
          ...
          const creditWeight = dirWeight * hopMultiplier * edgeDataWeight;
          ...
          expansion.push({
            ...
            creditWeight,
            path: [...path], // Clone to prevent shared-reference mutation issues
          });
  ...
  scoreCourseRelevance(course, targetTagIds) {
    ...
    // Use cached course metadata for fast lookups
    const { tagSet, suffixes, geminiTags } = this._getCourseMetadata(course);
    const targetIdsUnique = [...new Set(targetTagIds.map(t => t.toLowerCase()))];
    const topContributors = [];

    // ---- 1. Direct tag matches (highest weight: 25 pts each) ----
    let directOverlap = 0;
    for (const target of targetIdsUnique) {
      ...
      if (suffixes.has(targetSuffix)) {
        directOverlap += 15;
        // Find matching course tag for contribution reporting
        const match = Array.from(tagSet).find(ct => ct.endsWith(`.${targetSuffix}`) || ct === targetSuffix);
        topContributors.push({
          sourceQueryTagId: target,
          targetCourseTagId: match || target,
          path: [],
          contribution: 15,
        });
      }
    ...
    for (const tagId of targetIdsUnique) {
      ...
      for (const entry of expansion) {
        ...
        if (tagSet.has(entry.neighborId)) {
          matched = true;
        } else if (suffixes.has(entry.neighborSuffix)) {
          matched = true;
        }

        if (matched) {
          const credit = 5 * entry.creditWeight;
